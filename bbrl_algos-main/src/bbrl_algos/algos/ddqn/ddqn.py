...
def compute_critic_loss(
    ...
    # Determine the action that maximizes the Q-value using the online network
    best_actions = q_values[1].argmax(dim=-1).unsqueeze(-1)

    # Use this action to get the Q-value from the target network
    max_q = q_target[1].gather(dim=-1, index=best_actions).squeeze(-1).detach()

    # Compute the target value for Q-learning update
...
def run_dqn(cfg, logger, trial=None):
    ...
    if cfg.collect_stats:
        ...
        fo.flush()
        fo.close()
    torch.save(best_agent.agent.agents[1].model,
               "E:/sorbonne/IAR/mini_projet_nouveau/bbrl_algos-main/src/bbrl_algos/algos/" + cfg.gym_env.env_name + "_ddqn.pt")
    return best_reward
